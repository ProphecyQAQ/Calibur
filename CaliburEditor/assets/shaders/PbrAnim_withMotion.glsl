...
void main()
{
	...
	for (int i = 0; i < MAX_BONE_INFLUENCE; i++)
	{
		...
		boneTransform += u_FinalBonesMatrices[a_BoneIDs[i]] * a_Weights[i];
	}
	
	vec4 worldPosition = u_Transform * boneTransform * vec4(a_Position, 1.0);
	...
	vec3 T = normalize(vec3(u_Transform * boneTransform * vec4(a_Tangent,   0.0)));
	vec3 B = normalize(vec3(u_Transform * boneTransform * vec4(a_Bitangent, 0.0)));
	vec3 N = normalize(vec3(u_Transform * boneTransform * vec4(a_Normal,    0.0)));
	...
	jitterMat[2][0] += jitter.x * 1.0f;
	jitterMat[2][1] += jitter.y * 1.0f;

	gl_Position = jitterMat * u_ViewProjection * worldPosition;

	// Calculate new and old pos
	Output.newPos = u_ViewProjection * worldPosition;
	Output.oldPos = u_PreProjection * u_PreView * u_PreTransform * boneTransform *  vec4(a_Position, 1.0);
}
