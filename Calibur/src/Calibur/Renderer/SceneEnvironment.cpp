...
namespace Calibur
{
	...
	*/
	...
	{
		// Set up the box
		Ref<VertexBuffer> vbo = VertexBuffer::Create(boxVertices, sizeof(boxVertices));
		Ref<IndexBuffer> ibo = IndexBuffer::Create(boxIndices, sizeof(boxIndices));
		...
		Ref<UniformBuffer> camera = UniformBuffer::Create(sizeof(glm::mat4), 30);
		...
		for (size_t i = 0; i < 6; i++)
		{
			...
			camera->SetData(&viewProjecion, sizeof(glm::mat4));
			m_CaptureFramebuffer->SetRenderTargetToCubeMap(skyboxId, i);

			RenderCommand::Clear();

			Renderer::Submit(m_EquirectangularToCubemapShader, vao);
		}
